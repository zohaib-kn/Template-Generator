/**
 * app/api/sop/import/route.ts
 *
 * Backend endpoint for Statement of Purpose / Cover Letter Document Import.
...
import { processSopUpload } from "@/services/import/sopImportService";
  ...
  try {
    ...
    if (!file) {
      return NextResponse.json(
        { success: false, error: "No SOP document was uploaded." },
        { status: 400 }
      );
    }
    ...
    if (!isValidMagicBytes(buffer, mimeType, fileName)) {
      return NextResponse.json(
        {
          success: false,
          error: "The file format does not match its extension. Please upload a genuine PDF or DOCX file.",
        },
        { status: 400 }
      );
    }
    ...
    if (rawCrmSnapshot) {
      try {
        ...
        console.warn("[api/sop/import] Could not parse CRM snapshot:", err);
      }
    }
    ...
    console.error("[api/sop/import] Error:", err);
    const message =
      err instanceof Error
        ? err.message
        : "Failed to process SOP import. Please try again.";
