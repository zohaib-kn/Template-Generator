...
interface UsePdfGeneratorReturn {
  ...
  generate: (data: DocumentData, options?: PdfNamingOptions) => Promise<void>;
}
...
export function usePdfGenerator(): UsePdfGeneratorReturn {
  ...
  const generate = useCallback(
    async (data: DocumentData, options?: PdfNamingOptions) => {
      ...
      try {
        await generateDocumentPdf(data, options);
        setStatus("idle");
      } catch (err) {
        const msg =
