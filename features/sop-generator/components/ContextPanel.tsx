...
export function ContextPanel({ ctx, validationIssues }: ContextPanelProps) {
  ...
  return (
    <aside
      className="w-80 xl:w-88 flex-shrink-0 flex flex-col border-l border-slate-200 bg-white overflow-hidden"
      aria-label="Context panel"
    >
      {/* Tab bar */}
      <div className="flex border-b border-slate-200 flex-shrink-0 px-4 bg-white">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            id={`context-tab-${tab.id}`}
            onClick={() => setActiveTab(tab.id)}
            className={`flex-1 py-3 text-xs tracking-normal transition-colors relative flex items-center justify-center gap-1.5
                        ${
                          activeTab === tab.id
                            ? "text-[#096491] font-semibold border-b-2 border-[#096491]"
                            : "text-slate-500 hover:text-slate-800 font-medium border-b-2 border-transparent"
                        }`}
        ...
        {activeTab === "validation" && (
          <>
            {validationIssues.length === 0 ? (
              <div className="rounded-xl bg-[#F0FDF4] border border-[#DCFCE7] px-4 py-6 text-center">
                <p className="text-xl mb-1 text-emerald-600">✓</p>
                <p className="text-xs font-semibold text-emerald-800">
                  All checks passed
                </p>
                <p className="text-[11px] text-emerald-600 mt-0.5">
                  No validation issues found in document context.
                </p>
              </div>
            ) : (
              <div className="space-y-2.5">
                {validationIssues.map((issue) => {
                  ...
                  return (
                    <div
                      key={issue.id}
                      className={`rounded-lg border px-3 py-2.5 ${cfg.bg}`}
                    >
                      <div className="flex items-start gap-2">
                        <span
                          className={`font-bold text-xs flex-shrink-0 mt-0.5 ${cfg.color}`}
                          aria-hidden="true"
                        >
                          {cfg.icon}
                        </span>
                        <div>
                          <p className={`text-xs leading-relaxed font-medium ${cfg.color}`}>
                            {issue.message}
                          </p>
                          {issue.field && (
...
function ContextRow({
  ...
  return (
    <div className="flex gap-2 items-baseline text-xs">
      <span className="text-[11px] text-slate-400 w-[40%] flex-shrink-0">
        {label}
      </span>
      <span
        className={`text-xs font-medium leading-snug flex-1 break-words ${
          isEmpty
            ? "text-rose-400 italic font-normal"
            : highlight
            ? "text-[#096491] font-semibold"
            : "text-slate-800"
        }`}
      >
